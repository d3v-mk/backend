...
async def call(mesa_id: int, db: Session = Depends(get_db), current_user: Usuario = Depends(get_current_user)):
    mesa = db.query(Mesa).filter_by(id=mesa_id).first()
    if not mesa:
        raise HTTPException(status_code=404, detail="Mesa não encontrada.")
    
    acoes = ExecutorDeAcoes(mesa, db)
    await acoes.acao_call(current_user.id)
...
async def allin(mesa_id: int, db: Session = Depends(get_db), current_user: Usuario = Depends(get_current_user)):
    mesa = db.query(Mesa).filter_by(id=mesa_id).first()
    if not mesa:
        raise HTTPException(status_code=404, detail="Mesa não encontrada.")
    
    controlador = ExecutorDeAcoes(mesa, db)
    await controlador.acao_allin(current_user.id)

    return {"msg": f"Jogador {current_user.id} foi ALL-IN"}


@router.post("/{mesa_id}/check")
async def check(mesa_id: int, db: Session = Depends(get_db), current_user: Usuario = Depends(get_current_user)):
    mesa = db.query(Mesa).filter_by(id=mesa_id).first()
    if not mesa:
        raise HTTPException(status_code=404, detail="Mesa não encontrada.")
    
    controlador = ExecutorDeAcoes(mesa, db)
    await controlador.acao_check(current_user.id)

    return {"msg": f"Jogador {current_user.id} deu CHECK"}


@router.post("/{mesa_id}/fold")
async def fold(mesa_id: int, db: Session = Depends(get_db), current_user: Usuario = Depends(get_current_user)):
    mesa = db.query(Mesa).filter_by(id=mesa_id).first()
    if not mesa:
        raise HTTPException(status_code=404, detail="Mesa não encontrada.")
    
    controlador = ExecutorDeAcoes(mesa, db)
    await controlador.acao_fold(current_user.id)

    return {"msg": f"Jogador {current_user.id} deu FOLD"}


@router.post("/{mesa_id}/raise")
async def raise_aposta(mesa_id: int, valor: float, db: Session = Depends(get_db), current_user: Usuario = Depends(get_current_user)):
    mesa = db.query(Mesa).filter_by(id=mesa_id).first()
    if not mesa:
        raise HTTPException(status_code=404, detail="Mesa não encontrada.")
    
    controlador = ExecutorDeAcoes(mesa, db)
    await controlador.acao_raise(current_user.id, valor)
...
async def sair_da_mesa(
    ...
    await controlador.sair_da_mesa(current_user)
